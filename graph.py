...
def plotter():
    ...
    pattern     =  re.compile("num_values\s+=\s+(\d+).+in\s+(\d+.\d+)")
    ...
    for txt in files:
        with open(txt) as fd:
            ...
            values, runtimes  = zip(*[(float(match.group(1)), float(match.group(2))) for line in fd.readlines() for match in [pattern.search(line)] if match])
    ...
    fig = make_subplots(rows=2, cols=1, subplot_titles=("Values and Avg Runtimes", 'Values and Speedup'))

    fig.add_trace(
        go.Scatter(x=values, y=average_runtimes),
        row=1, col=1
    )

    fig.add_trace(
        go.Scatter(x=values, y=average_speedups),
        row=2, col=1
    )

    # edit axis labels
    fig['layout']['xaxis']['title']='Num of Values'
    fig['layout']['xaxis2']['title']='Num of Values'
    fig['layout']['yaxis']['title']='Average Runtime (s)'
    fig['layout']['yaxis2']['title']='Average Speedup'
    fig.show()
    # HTML
    pio.write_html(fig, 'plotly_graph.html', auto_open=False)
